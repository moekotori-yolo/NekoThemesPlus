    ...
    internal static class NekoThemeImportExport
    {
        ...
        public static bool Import(string path, out string message)
        {
            ...
            try
            {
                ...
                if (file.Length > MaximumEmbeddedImageBytes * 2L)
                {
                ...
                if (data == null || data.format != Format || data.schemaVersion != SchemaVersion)
                {
        ...
        private static bool TryEmbedBackground(string path, ThemeData data, out string message)
        {
            ...
            {
                data.backgroundExtension = string.Empty;
                data.backgroundBase64 = string.Empty;
                return true;
            }

            if (!File.Exists(path))
            {
                message = NekoThemesPlusLocalization.Text("背景图片不存在，无法导出完整主题。", "The background image is missing, so the complete theme cannot be exported.");
                return false;
            }
            ...
            {
                message = NekoThemesPlusLocalization.Text("主题只能内嵌 PNG/JPG/JPEG 背景。", "A theme can only embed a PNG/JPG/JPEG background.");
                return false;
            }
            ...
            {
                message = NekoThemesPlusLocalization.Text("背景图片超过 64 MB，无法内嵌。", "The background image exceeds the 64 MB embedding limit.");
                return false;
            }

            data.backgroundExtension = extension;
            data.backgroundBase64 = Convert.ToBase64String(File.ReadAllBytes(path));
            return true;
        }
        ...
        private static bool TryRestoreBackground(ThemeData data, out string path, out string message)
        {
            path = string.Empty;
            message = string.Empty;
            if (string.IsNullOrEmpty(data.backgroundBase64))
            {
                return true;
            }

            string extension = (data.backgroundExtension ?? string.Empty).ToLowerInvariant();
            if (extension != ".png" && extension != ".jpg" && extension != ".jpeg")
            {
            ...
            try
            {
                bytes = Convert.FromBase64String(data.backgroundBase64);
            }
